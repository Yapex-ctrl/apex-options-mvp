  ...
  return (
    <main className="container" dir="rtl">
      <section className="hero">
        ...
        <p className="small">MVP جاهز للنشر كرابط: تحليل عقود واستراتيجيات أوبشن بدون شارتات.</p>
      </section>
      ...
      {data && (
        <>
          <div className="card">
            <h2>{data.ticker} — Market State</h2>
            <div className="grid">
              ...
              <div className="kpi"><div className="small">Earnings</div><b>{data.earnings_date}</b></div>
          ...
          <div className="card">
            ...
            <div className="tableWrap"><table>
              <thead><tr><th>Symbol</th><th>Type</th><th>Strike</th><th>DTE</th><th>Mid</th><th>IV</th><th>Delta</th><th>Theta</th><th>Vol</th><th>OI</th></tr></thead>
              <tbody>{data.top_contracts.map((c: any) => (
                <tr key={c.symbol}><td>{c.symbol}</td><td>{c.type}</td><td>{c.strike}</td><td>{c.dte}</td><td>{c.mid}</td><td>{Math.round(c.iv*100)}%</td><td>{c.delta}</td><td>{c.theta}</td><td>{c.volume}</td><td>{c.open_interest}</td></tr>
              ))}</tbody>
            </table></div>
          </div>
